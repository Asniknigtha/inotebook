...
const Notes = (props) => {
  ...
  const handleClose = () => setOpen(false);
    ...
    const {notes,getNotes, setNotes, addNotes ,editNotes} = context;
    useEffect(()=>{
...
<AddNotes showAlert={showAlert}></AddNotes>
    ...
    <div>
      <Button  style={{display: 'none'}} ref={ref} onClick={handleOpen}>Open modal</Button>
      <Modal
        open={open}
        onClose={handleClose}
    ...
    >
      ...
      <TextField id="tag" label="Tag" name="tag"  onChange={onChange} value={note.tag} variant="outlined" />
          ...
          <Button  disabled={note.title.length<5 || note.description.length<5} onClick={handleClick}>Save Notes</Button>
        </Box>
